...
class ExternalSolver(object):
    ...
    def launch(self, prog, game):
        ...
        """
        game_specification = game.write(format='native').encode('latin-1').decode('unicode_escape')[2:-2]
        p = subprocess.run("%s -q" % prog, shell=True,
                             input=game_specification, stdout=subprocess.PIPE, encoding='ascii')
        return p.stdout

    def _parse_output(self, out_string, game, rational, extensive=False):
        profiles = []
...
class ExternalEnumPureSolver(ExternalSolver):
    ...
    """
    # Nicolas: added subgame perfect option
    def solve(self, game, use_strategic=False, subgame_perfect=False):
        ...
        if use_strategic and game.is_tree:
            command_line += " -S"
        if subgame_perfect and game.is_tree:
            command_line += " -P"
